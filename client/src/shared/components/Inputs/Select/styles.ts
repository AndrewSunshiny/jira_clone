...
interface StyledSelectProps extends ComponentProps<'div'> {
  $hasIcon?: boolean;
  $invalid?: boolean;
}
export const StyledSelect = styled.div<StyledSelectProps>`
  ...
  ${(props) => `
		${props.$hasIcon && 'padding-left: 25px;'}
		${
      props.$invalid
      && css`
        &,
...
export const StyledIconComponent = styled(Icon)`
  position: absolute;
  top: 12px;
  left: 14px;
  font-size: 16px;
...
export const ChevronIconComponent = styled(Icon)`
  position: absolute;
  top: 10px;
  right: 11px;
  font-size: 18px;
...
export const ClearIconComponent = styled(Icon)`
  position: absolute;
  top: 4px;
  right: 7px;
  padding: 5px;
